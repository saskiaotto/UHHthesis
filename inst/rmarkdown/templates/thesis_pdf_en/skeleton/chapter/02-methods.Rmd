...
If you want to add external images here, e.g. to show the sampling site as in Fig. \@ref(fig:location), use the `knitr::include_graphics()` function. You should always provide a figure legend, which you can define in the code chunk option `fig.cap` inside the curly brackets at the very beginning. If you want the image to take less then 100% of the page with use the option `out.width` as demonstrated here:
...
Tables require also a label and table caption for cross-referencing. But here, the cross-reference contains a `tab:` in `\@ref(tab:<name>)`) instead of a `fig:`. Also, captions of tables produced with R cannot be set in the chunk options as for figures but in the R functions directly (see examples in the [Results]).
